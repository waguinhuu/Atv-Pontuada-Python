...
@pytest.fixture
def advogado_valido():
    return Advogado(123, "Wagner", "71982345670", "wagner@gmail.com",
                      ...
                      "12435533", "4556", Setor.ENGENHARIA.nome, 2600, "123456")


"""VALIDAÇÕES DOS DADOS DO USUARIO"""

def validar_id(advogado_valido):
    assert advogado_valido.id == 123

def validar_nome(advogado_valido):
    assert advogado_valido.nome == "Wagner"

def validar_telefone(advogado_valido):
    assert advogado_valido.telefone == "71982345670"

def validar_email(advogado_valido):
    assert advogado_valido.email == "wagner@gmail.com"

def validar_cpf(advogado_valido):
    assert advogado_valido.cpf == "12345678900"

def validar_rg(advogado_valido):
    assert advogado_valido.rg == "12435533"

def validar_matricula(advogado_valido):
    assert advogado_valido.matricula == "4556"

def validar_estado_civil(advogado_valido):
    assert advogado_valido.estado_civil == EstadoCivil.SOLTEIRO.nome

def validar_sexo(advogado_valido):
    assert advogado_valido.sexo == Sexo.MASCULINO.nome

def validar_data_de_nascimento(advogado_valido):
    assert advogado_valido.data_de_nascimento == "05/06/2005"

def validar_setor(advogado_valido):
    assert advogado_valido.setor == Setor.ENGENHARIA.nome

def validar_salario(advogado_valido):
    assert advogado_valido.salario == 2600

def validar_oab(advogado_valido):
    assert advogado_valido.oab == "123456"


def validar_endereco(advogado_valido):
    assert advogado_valido.endereco.logadouro == "Drena"
    assert advogado_valido.endereco.numero == "93"
    assert advogado_valido.endereco.complemento == "casa"
    assert advogado_valido.endereco.cidade == "Salvador"
    assert advogado_valido.endereco.cep == "43900-000"
    assert advogado_valido.endereco.uf == UnidadeFederativa.BAHIA.nome


""" TESTES PARA CPF"""
def test_cpf_vazio():
    with pytest.raises(ValueError, match="O CPF não deve estar vazio."):
        Advogado(123, "Wagner", "71982345670", "wagner@gmail.com",
                      Endereco("Drena", "93", "casa", "43900-000", "Salvador", UnidadeFederativa.BAHIA.nome), 
...
def test_oab_tipo_invalido():
    with pytest.raises(TypeError, match="O OAB deve ser em texto."):
        Advogado(123, "Wagner", "71982345670", "wagner@gmail.com",
                      Endereco("Drena", "93", "casa", "43900-000", "Salvador", UnidadeFederativa.BAHIA.nome), 
                      Sexo.MASCULINO.nome, EstadoCivil.SOLTEIRO.nome, "05/06/2005", "12345678900", 
                      "12435533", "4556", Setor.ENGENHARIA.nome, 2600, 123456)